export default [

    {
        name: { firstName: 'Gato', surName: 'Abelha' },
        phone: ['123987123'],
        _id: "5b708fbb11942e1e83a7e088",
        birthday: "1999-04-15T00:00:00.000Z",
        address: { street: 'rua da dona vesa', number: 409, district: 'pais dos gatos', city: 'gatolandia', state: 'destrito dos gatos', country: 'gatinia', cep: 6666 },
        rg: 12039812309,
        sexuality: 'Masculino',
        education:'zkjdskdjdk',
        cpf: '123098123',
        email: 'Gatinhobonitinho123@bol.com',
        nacionality: 'Koreano',
        maritalStatus: ' viuvo de 3',
        children: [],
        observations: 'meio castrado',
        photo: 'img_users/adult/5b708fbb11942e1e83a7e088.png',
        crianca: [{ Nome: 'maria', parentesco: 'tio avo', id: 0 }],
        workPortfolio: {
            number: 'sss',
            series: 'sss',
            state: 'sss',
            PIS_PASEP: 'sss',
            dateIssue: 'sss',
            placeIssue: 'sss',
        },
        kinship: {
            fatherName: 'cccfffffffffffffffffffff',
            motherName: 'dsfsfssffffffffffffffffff'
        },
        birthplace: {
            city: 'dddd',
            state: 'safad',

        },
        electionTitle: {
            number: 'zfsdfsd',
            zone: 'zfsdfsd',
            section: 'zfsdfsd',
            state: 'zfsdfsd',
        },
        militaryReservist: {
            number: 'zfsdfsd',
            series: 'dads',
            category: 'dads',
        },
        passport: {
            number: 'zfsdfsd',
            type: 'asfa',
            issuingCountry: 'asfa',
            dateIssue: 'asfa',
            expirationDate: 'asfa',
        },
        cnh: {
            record: 'dsds',
            category: 'dsds',
            expirationDate: 'dsds',
            comments: 'dsds',
            placeIssue: 'dsds',
            dateIssue: 'dsds',

        },
        employeeData: {
            officialPosition: 'dsds',
            admissionDate: 'dsds',
            resignationDate: 'dsds',
            reasonResignation: 'dsds',
            record: 'dsds',
            state: 'dsds',

        },
        __v: 0
    },

    {
        name: { firstName: 'Matheus', surName: 'Felipe' },
        phone: ['84555555555'],
        _id: "5b75ee330bb2c51fe2de34e8",
        birthday: new Date("1997-07-05T00:00:00.000Z").toDateString(),
        address: [],
        rg: 222222222,
        cpf: '00000000000',
        email: 'Matt@hotmail.com',
        nacionality: 'Brasileiro',
        sexuality: 'Masculino',
        maritalStatus: 'Solterio(a)',
        children: [],
        observations: 'Observações',
        photo: 'img_users/adult/5b75ee330bb2c51fe2de34e8.png',
        __v: 0

    },

]