...
class DashBoard extends React.Component {
	...
	mudar(){
		if(!this.state.colaps){
			this.setState({colaps:true});

		}else{
			this.setState({colaps:false});
		}}
		mudar2(){
			if(!this.state.adultospopup){
				this.setState({adultospopup:true});

			}else{
				this.setState({adultospopup:false});
			}

		}
		mudar3(){
			if(!this.state.Eventospopup){
				this.setState({Eventospopup:true});

			}else{
				this.setState({Eventospopup:false});
			}            

		}
		mudar4(){

			if(!this.state.noticiaspopup){
				this.setState({noticiaspopup:true});

			}else{
				this.setState({noticiaspopup:false});
			}
		...
		render() {
			...
			var byNoticias = function(events) {
				return(
					<tr key={events.toString()}>
					<td>{events.Titulo}</td>
